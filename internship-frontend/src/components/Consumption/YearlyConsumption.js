...
const YearlyConsumption = (props) => {
    ...
    const fetchData = async(year)=>{
        setloading(true);
        let response = await fetch (`https://powersense.in/api/${props.type}_consumption?start_datetime=${year}`,{
            method :'GET',
            headers :{
  ...
  return (
    ...
    <div className='container contain'>
      <h1 className='my-3'>Yearly Consumption</h1>
          <form onSubmit={handleSubmit}>
            <div className="form-floating" style={{marginTop:"35px", marginBottom:"15px"}}>
                <input type='year' style={{height: "100px"}} className="form-control" placeholder="Leave a comment here" id="floatingTextarea2" onChange={handleYear} required={true}></input>
