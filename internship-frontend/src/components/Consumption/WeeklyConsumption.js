...
const WeeklyConsumption = (props) => {
    ...
    const fetchData = async(week)=>{
        setloading(true);
        let response = await fetch (`https://powersense.in/api/${props.type}_consumption?start_datetime=${week}`,{
            method :'GET',
            headers :{
    ...
    console.log(ChartData);
     ...
     const handleWeek = (e)=>{
        let week1 = moment(e.target.value).toDate();
        let week2 = moment(week1).format("YYYY-MM-DD");
        setWeek(week2);
     }
     const handleSubmit = (e) =>{
        e.preventDefault();
        if(!week){
            alert('Please enter correct week and year')
        }
        else{
            fetchData(week);
            console.log(week)
  ...
  return (
   ...
   <div className='main' style={{height:"130vh"}}>
    <div className='container contain'>
      <h1 className='my-3'>Weekly Consumption</h1>
          <form onSubmit={handleSubmit}>
            <div className="form-floating" style={{marginTop:"35px", marginBottom:"15px"}}>
                <input type='week' style={{height: "100px"}} className="form-control" placeholder="Leave a comment here" id="floatingTextarea2" onChange={handleWeek} required={true}></input>
